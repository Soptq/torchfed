...
class P2PNode(Node):
    ...
    def __init__(self, host, port, id=None, receive_callback=None, response_callback=None, max_connections=0):
        super(P2PNode, self).__init__(host, port, id, callback=None, max_connections=max_connections)
        self.receive_callback = receive_callback
        self.response_callback = response_callback

    def bootstrap(self, host: str, port: int, reconnect: bool = False) -> bool:
        bootstrap_server_connected = self.connect_with_node(host, port, reconnect=reconnect)
    ...
    def node_message(self, connected_node, data):
        ...
        if data['type'] == P2PQueryType.GET_PEERS_QUERY:
            all_nodes_info = [{"host": node.host, "port": node.port, "id": node.id} for node in self.all_nodes]
            connected_node.send(construct_query(P2PQueryType.GET_PEERS_RESP, all_nodes_info))
        ...
        if data['type'] == P2PQueryType.REGULAR_MSG:
            ...
            if response is not None:
                connected_node.send(construct_query(P2PQueryType.REGULAR_MSG_RESP, response.serialize()))
